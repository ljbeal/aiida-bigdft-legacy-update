...
``aiida-bigdft-new`` comes with a ``.github`` folder that contains continuous integration tests on every commit using `GitHub Actions <https://github.com/features/actions>`_. It will:
...
Once you're happy with your documentation, it's easy to host it online on ReadTheDocs_:
 ...
 #. Import your ``aiida-bigdft-new`` repository (preferably using ``aiida-bigdft-new`` as the project name)

The documentation is now available at `aiida-bigdft-new.readthedocs.io <http://aiida-bigdft-new.readthedocs.io/>`_.
