...
new aiida bigdft plugin for python 3 and aiida 2.0
...
* [`aiida_bigdft_new/`](aiida_bigdft_new/): The main source code of the plugin package
  * [`data/`](aiida_bigdft_new/data/): A new `DiffParameters` data class, used as input to the `DiffCalculation` `CalcJob` class
  * [`calculations.py`](aiida_bigdft_new/calculations.py): A new `DiffCalculation` `CalcJob` class
  * [`cli.py`](aiida_bigdft_new/cli.py): Extensions of the `verdi data` command line interface for the `DiffParameters` class
  * [`helpers.py`](aiida_bigdft_new/helpers.py): Helpers for setting up an AiiDA code for `diff` automatically
  * [`parsers.py`](aiida_bigdft_new/parsers.py): A new `Parser` for the `DiffCalculation`
